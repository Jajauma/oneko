 ...
 *
 *	Changelog:
 *	-20170402 Added ClickThrough (ShapeInput-Pixmap) by Simon Ullrich Richter
 */
...
int	NoClickThrough = NOTDEFINED;	/*   noclick	*/
...
InitBitmapAndGCs()
{
    ...
    InputPixmap = XCreatePixmapFromBitmapData(theDisplay, theRoot,
    		BitmapGCDataTable->PixelPattern[NekoMoyou],
...
GetResources()
{
  ...
  if (NoClickThrough == NOTDEFINED) {
    if ((resource = NekoGetDefault("noclick")) != NULL) {
      NoClickThrough = IsTrue(resource);
    }
  }
...
#ifdef SHAPE
  if ( (!NoShape || !NoClickThrough) && XShapeQueryExtension(theDisplay,
				       &event_base, &error_base) == False) {
    fprintf(stderr, "Display not suported shape extension, disabling shape and click-through.\n");
    NoShape = True;
    NoClickThrough = True;
				       }
#endif //SHAPE
...
#ifdef SHAPE
   if (NoClickThrough == False) {
	XShapeCombineMask(theDisplay, theWindow, ShapeInput,
			  0, 0, InputPixmap, ShapeSet);
   }
#endif //SHAPE
...
"-noclick               : Block mouse pointer from clicking through neko.",
    ...
    char	*theDisplayName;
{
  ...
  for (ArgCounter = 0; ArgCounter < argc; ArgCounter++) {
    ...
    else if (strcmp(argv[ArgCounter], "-noclick") == 0) {
      NoClickThrough = True;
    }
